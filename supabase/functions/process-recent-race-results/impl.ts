 ...
 */ 
export async function upsertRaceResults(supabaseAdmin, season, round) {
    console.log(`Fetching results for ${season} round ${round}...`);
    // 1. Fetch race results from API
    const response = await fetch(`https://api.jolpi.ca/ergast/f1/${season}/${round}/results.json`).then((res)=>res.json());
    // Get the race data
    const raceData = response.MRData.RaceTable.Races[0];
    if (!raceData) {
      console.log(`No results found for ${season} round ${round}`);
      return;
    }
    ...
    const { data: raceData2 } = await supabaseAdmin.from('races').select('id').eq('name', raceName).single();
    if (!raceData2) {
    ...
    for (const result of results){
      ...
      const { data: driverData } = await supabaseAdmin.from('drivers').select('id').eq('code', result.Driver.code).single();
      if (!driverData) {
        console.error(`Driver not found in database: ${result.Driver.code}`);
        continue;
      }
      const { error } = await supabaseAdmin.from('race_results').insert({
        race_id: raceId,
   ...
   */ export async function processRecentRaces(supabaseAdmin) {
    console.log('Checking for recent races without results...');
    // Get date for 24 hours ago
    const lookBack = new Date();
    lookBack.setDate(lookBack.getDate() - 20);
    // Find races that ended in the last 24 hours
    const { data: recentRaces, error } = await supabaseAdmin.from('races').select('*').lt('date', new Date().toISOString()).gte('date', lookBack.toISOString());
    if (error) {
    ...
    for (const race of recentRaces){
      ...
      const { data: results, error: resultsError } = await supabaseAdmin.from('race_results').select('id').eq('race_id', race.id).limit(1);
      if (resultsError) {
      ...
      if (!results || results.length === 0) {
        ...
        try {
          await upsertRaceResults(supabaseAdmin, season, round);
        } catch (err) {
